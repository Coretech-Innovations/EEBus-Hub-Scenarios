...
func main() {
	...
	// add the uncontrollable device (while the simulation is stopped)
	...
	for {
		...
		resp = sendRequest("GET", endPoint, nil)
		{
			...
			if len(limitsCasted) < 3 {
				fmt.Printf("per-phase limit for EV%d :  not available yet\n", ev1ID)
			} else {
				val := [3]float64{limitsCasted[0].Value.GetValue(), limitsCasted[1].Value.GetValue(), limitsCasted[2].Value.GetValue()}
